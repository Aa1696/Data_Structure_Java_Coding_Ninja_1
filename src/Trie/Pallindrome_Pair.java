...
class pallindrome_TrieNode {
    char data;
    boolean isTerminating;
    TrieNode children[];
    int childCount;

    public pallindrome_TrieNode(char data) {
        this.data = data;
        isTerminating = false;
...
public class Pallindrome_Pair {
    private TrieNode root;
    public int count;

    public Pallindrome_Pair() {
        root = new TrieNode('\0');
    }
    ...
    public boolean isPalindromePair(ArrayList<String> words) {
        ...
        for(int i=0;i<words.size();i++){
            ...
            String st=reverse(words.get(i));
            s.add(st);
        }
        for(int j=0;j<s.size();j++){
            boolean br=search(s.get(j));
            if(br==true){
                return true;
            }
        }
