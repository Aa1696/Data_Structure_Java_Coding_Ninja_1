...
public class Search_In_Trie {
    ...
    private boolean helper_search(TrieNode root,String word){
        if (word.length() == 0) {
            return root.isTerminating;
        }
        int word_index = word.charAt(0) - 'a';
        TrieNode nd = root.children[word_index];
        if (nd==null) {
            return false;
        }
        return helper_search(nd,word.substring(1));
    }
